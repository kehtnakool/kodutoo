...
class View(Tk):
    ...
    def generate_leaderboard(self, frame, data):
        ...
        for p in data:
            ##### muudetud osa
            #############################################################################
            #### annab vea, kui edetabeli fail puudub, lisasin testi
            try:
                ...
                my_table.insert(parent="", index="end", iid=str(x), text="",
                                values=("edetabel", "on", "veel", "tühi", ""))
            ####################################################################################
            #### miks hiljem pole vaja testida,
            #### kas kõige ülemine rida == ("edetabel", "on", "veel", "tühi", "")
            #### seda ei tea, töötab ilma selletagi
